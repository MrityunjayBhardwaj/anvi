...
| 19 | The always-loaded instruction surface growing a line at a time with nothing to say so — and a re-measurement that rewrote its own baseline, so "compare against the baseline" compared the surface with itself | `test/surface-limit.test.js` — the shipped `cognitive-os/` files are counted against `references/surface-limits.json` on every suite run; counting never writes the limits (only `--write-limits` does, in the same diff as the growth); the count uses no word list, so growth in new vocabulary still counts. **It limits size, not compliance** — the machine `CLAUDE.md` and `MEMORY.md` are reported by `scripts/surface-count.js`, never enforced, against limits kept in the project's store (`<store>/surface-limits.json`), since a machine's numbers in a shipped file would describe someone else's files on every other install |
